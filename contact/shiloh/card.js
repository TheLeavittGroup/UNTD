...
const initials = n => (n || 'R').split(/\s+/).filter(Boolean).slice(0, 2).map(x => x[0]).join('').toUpperCase() || 'R';
const first = n => (n && n !== 'Rep Name' ? n.split(/\s+/)[0] : 'rep');
const money0 = n => (Number.isFinite(n) ? n : 0).toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 0 });
const money2 = n => (Number.isFinite(n) ? n : 0).toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
function read() {
  return {
    name: $('setName').value.trim() || D.name,
...
function apply() {
  ...
  $('topCompany').textContent = c.company;
  $('repName').textContent = c.name;
  ...
  ['emailBtn', 'ctaEmail'].forEach(id => $(id).href = safeHref('mailto:', c.email));

  if (c.photo) {
    $('repPhoto').src = c.photo;
    $('photoBox').classList.add('has-img');
  } else {
    $('repPhoto').removeAttribute('src');
...
function lockedUrl() {
  ...
  u.hash = 'card=' + encodeURIComponent(q.toString());
  return u.toString();
}
...
function lockCard() {
  ...
  closeDrawer();
  show('Card locked. Tap the photo to share it.');
}
...
async function buildCardImage() {
  ...
  let g = ctx.createLinearGradient(0, 0, 1080, 1350);
  g.addColorStop(0, '#050506');
  g.addColorStop(.55, '#111722');
  g.addColorStop(1, '#050506');
  ctx.fillStyle = g;
  ctx.fillRect(0, 0, 1080, 1350);

  ctx.fillStyle = 'rgba(214,170,69,.18)';
  ctx.beginPath();
  ctx.arc(180, 90, 360, 0, Math.PI * 2);
  ctx.fill();

  ctx.fillStyle = 'rgba(68,200,255,.16)';
  ctx.beginPath();
  ctx.arc(920, 140, 300, 0, Math.PI * 2);
  ctx.fill();
  ...
  const im = await loadImage(c.photo);
  if (im) {
    cover(ctx, im, 120, 120, 840, 650);
  } else {
    let rg = ctx.createRadialGradient(540, 380, 30, 540, 380, 380);
    rg.addColorStop(0, '#ffe29a');
    rg.addColorStop(1, '#d6aa45');
    ctx.fillStyle = rg;
    ctx.fillRect(120, 120, 840, 650);
    ctx.fillStyle = '#140e04';
    ctx.beginPath();
    ctx.arc(540, 380, 170, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#f7f4ec';
    ctx.font = '900 110px Inter,Arial';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(initials(c.name), 540, 380);
  }
  ...
  ctx.fillText(c.name, 540, 870);
  ctx.fillStyle = '#ffe29a';
  ctx.font = '900 30px Inter,Arial';
  ctx.fillText((c.company || '').toUpperCase(), 540, 925);
  ctx.fillStyle = 'rgba(255,255,255,.82)';
  ctx.font = '700 34px Inter,Arial';
  ctx.fillText('Business card + helpful solar and battery resources', 540, 1000);
  ctx.fillStyle = '#d6aa45';
  ctx.font = '900 38px Inter,Arial';
  ctx.fillText(displayPhone(c.phone) || 'Phone number', 540, 1080);
  ctx.fillStyle = '#f7f4ec';
...
async function shareCard() {
  ...
  try {
    ...
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
      await navigator.share({ title: c.name + ' | ' + c.company, text: msg, files: [file] });
    } else if (navigator.share) {
      await navigator.share({ title: c.name + ' | ' + c.company, text: msg, url: link });
    } else {
      await navigator.clipboard.writeText(link);
      show('Card link copied.');
    }
  } catch (e) {
    show('Share cancelled. Tap the photo when ready.');
  }
}
...
function fileToPhoto(file) {
  ...
  r.onload = () => {
    ...
    img.onload = () => {
      ...
      photoData = canvas.toDataURL('image/jpeg', .62);
      $('setPhoto').value = '';
      apply();
      $('status').textContent = 'Photo added. Tap Lock My Card.';
    };
    img.src = r.result;
  };
...
function load() {
  const h = location.hash.startsWith('#card=') ? decodeURIComponent(location.hash.slice(6)) : '';
  const q = new URLSearchParams(h || location.search);

  if (q.size) {
    put({
      name: q.get('name') || D.name,
...
function calc() {
  ...
  const bill = Math.max(0, num('bill'));
  ...
  for (let y = 1; y <= yrs; y++) {
    const u = bill * Math.pow(1 + uEsc, y - 1);
    const s = solarStart * Math.pow(1 + sEsc, y - 1);
    uT += u * 12;
    sT += s * 12;
    if (y === yrs) last = u;
  }

  $('yearsDisplay').textContent = yrs;
  $('snapYear').textContent = yrs;
  $('utilTotal').textContent = money0(uT);
  $('solarTotal').textContent = money0(sT);
  $('differenceLabel').textContent = money0(uT - sT);
  $('selMonthlyUtility').textContent = money2(last);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('lockCard').onclick = lockCard;
  $('photoBox').onclick = shareCard;
  $('photoBox').onkeydown = e => {
  ...
  ['bill', 'solarPayment', 'yearsRange', 'solarEsc'].forEach(id => {
    const el = $(id);
